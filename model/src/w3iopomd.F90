...
9004 FORMAT (' TEST W3IOPE :',F8.1,2(F7.2,F7.1))
  ...
  !> @author Edward Hartnett  @date 1-Nov-2023
  !>
  SUBROUTINE W3IOPON_READ(IOTST, IMOD_IN, filename, timestep_only, ncerr)
    USE NetCDF
    USE W3ODATMD, ONLY: W3DMO2
    ...
    INTEGER, INTENT(IN)           :: timestep_only
    character(*), intent(in) :: filename
    integer, intent(inout) :: ncerr
    INTEGER :: IGRD,MK,MTH,i
    integer :: fh
    ...
    CHARACTER(LEN=124) :: full_filename
    ...
    END IF
      ...
      ! Logic to determine whether to read from multiple files or a single file
    IF (timestep_only .EQ. 1) THEN
      ! Loop over all available timesteps
      DO i = 1, d_time_len
        ! Create TIMETAG for file name using YYYYMMDD.HHMMS prefix
        WRITE(TIMETAG,"(i8.8,'.'i6.6)")TIME(1),TIME(2)
        full_filename = filename(:LEN_TRIM(filename))//TIMETAG//'.out_pnt.'//FILEXT(:LEN_TRIM(FILEXT))//'.nc'
      
        ! Call the internal read function for each file
        CALL W3IOPON_READ_SINGLE(IOTST, IGRD, full_filename, i, ncerr)
        IF (ncerr /= 0) RETURN
      END DO
    ELSE
      ! Read from a single file
      CALL W3IOPON_READ_SINGLE(IOTST, IGRD, filename, 1, ncerr)
      IF (ncerr /= 0) RETURN
    END IF
  END SUBROUTINE W3IOPON_READ

  SUBROUTINE W3IOPON_READ_SINGLE(IOTST, IGRD, filename, timestep, ncerr)
    USE NetCDF
    USE W3ODATMD, ONLY: W3DMO2
    USE W3WDATMD, ONLY: TIME
    USE W3GDATMD, ONLY: NTH, NK, NSPEC, FILEXT
    ...
    !print *, "Attempting to open file:", trim(filename)
    ! Open the netCDF file.
    ncerr = nf90_open(filename, NF90_NOWRITE + NF90_NETCDF4, fh)
    if (nf90_err(ncerr) .ne. 0) then
      print *, "Error opening file with NF90_NETCDF4 flag:", trim(nf90_strerror(ncerr))
      return
    endif
...
904 FORMAT (/' *** WAVEWATCH III ERROR IN W3IOPO :'/                &
  ...
  END SUBROUTINE W3IOPON_READ_SINGLE
  ...
  SUBROUTINE W3IOPON ( INXOUT, NDSOP, IOTST, IMOD)
    ...
    IF (INXOUT .EQ. 'READ') THEN
      CALL W3IOPON_READ(IOTST, IMOD, filename, OFILES(2), ncerr)
    ELSE
